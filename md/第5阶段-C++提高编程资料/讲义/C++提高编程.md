...
例如：查找find，拷贝copy，删除erase,  替换replace，插入insert
...
* `错误!!!   v4.assign(v1);    //直接将vector放入assign中会报错`
...
* `erase(iterator start, iterator end);`//删除迭代器从start到end之间的元素
* `clear();`                                                        //删除容器中所有元素
...
* `front(); `            //返回容器中第一个数据元素      *(v1.begin()) 和 front() 相同

* `back();`              //返回容器中最后一个数据元素  *(v1.end()-1) 和 back() 相同
...
* `assign(iterator beg, iterator end);`         //将[beg, end)区间中的数据拷贝赋值给本身。
* `错误!!!  d3.assign(d2); `    //直接将另一个deque放入会报错
* `assign(n, elem);`                                             //将n个elem拷贝赋值给本身。
...
* `insert(iterator pos,elem);`          //在pos位置插入一个elem元素的拷贝，返回新数据的位置。

* `insert(iterator pos,n,elem);`      //在pos位置插入n个elem数据，无返回值。

* `insert(iterator pos,iterator beg,iterator end);`    
...
* 利用算法实现对vector和deque容器进行排序



**算法：**

* `sort(iterator beg, iterator end)`  //对beg和end区间内元素进行排序   	默认升序
...
有 empty() 函数判断是否为空 为空返回true,不为空返回false
...
* `list(iterator beg,iterator end);`          //构造函数将[beg, end)区间中的元素拷贝给本身。
  * (l1.begin() + 1, l1.end() - 1) 会报错,vector和deque可以这样,list用的是指针,这样写是错误的,可以使用l1.begin() + + 代替
...
* `assign(iteratorbeg,iterator end);`        //将[beg, end)区间中的数据拷贝赋值给本身。
  *  `错误!!!  assign(l2);    不能直接将另一个list放进去,会报错`
  * `不能让 l2.begin() + 1,但是可以使用 ++或者--,如++l2.begin(),--l2.end()`
* `assign(n, elem);`              //将n个elem拷贝赋值给本身。
...
* set/multiset属于**关联式容器**，底层结构是用**二叉树**实现。map底层也是二叉树,set和map不支持随机访问
...
* `set(operator beg,operator end); ` //beg和end可以使用 ++ -- ,不能使用 +1 -1   //区间拷贝
...
* `erase(operator beg,operator end);`    //删除区间[beg,end)的所有元素 ，返回下一个元素的迭代器。
...
* `find(key);`                  //查找key是否存在,若存在，返回该键的元素的迭代器；若不存在，返回set.end();
  * `end()代表最后一个数据的下一个`
  * multiset可以找到多个,返回第一个
* `count(key);`                //统计key的元素个数结果,set不允许重复,结果是0或者1,对于multiset会有其他值
...
* map/multimap属于**关联式容器**，底层结构是用二叉树实现。set底层也是二叉树,set和map不支持随机访问
...
* `map<T1, T2> mp = {pair<T1,T2>(值1,值2),pair<T1,T2>(值1,值2),pair<T1,T2>(值1,值2),...};`    
  * ​                                                                //={}初始化
...
- `merge(iterator beg1, iterator end1, iterator beg2, iterator end2, iterator dest);  `
  ...
  // 注意: 两个容器必须是**有序的**,顺序必须一致,合并完之后结果是有序的,不然合并完结果是无序的
